...
window.onload = () => {
    ...
    function main() {
        ...
        async function initializeApp() {
            ...
            try {
                ...
                handleError("Failed to load your data. Please check your internet connection and try again.");
            }
        }
        ...
        async function renderLeaderboard() {
            ...
            dom.leaderboardContainer.innerHTML = users.map((user, i) => `
                <div class="list-item leaderboard-item ${user.id == userId ? 'is-user' : ''}">
                    <span class="leaderboard-rank">${i + 1}</span><div class="item-info leaderboard-info"><span class="leaderboard-name">${user.fullName || 'A User'}</span>
    ...
    try {
        ...
        // The ready() method tells the Telegram client that the web app is ready to be displayed.
        // It should be called when all the essential parts of your app's UI are loaded.
        tg.ready();
        // Now that we know Telegram is ready, we can start our main application logic.
        main();
    } catch (e) {
        console.error("Telegram Web App SDK is not available. This app can only be run inside Telegram.", e);
        document.body.innerHTML = `<h1 style="color: #ff4d4d; text-align: center; padding: 20px;">This app can only be run inside Telegram.</h1>`;
    }
};