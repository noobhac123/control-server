document.addEventListener('DOMContentLoaded', () => {
    ...
    const tg = window.Telegram || { WebApp: { initDataUnsafe: { user: { id: '123_LOCAL_TEST', first_name: 'Test', last_name: 'User', username: 'testuser', photo_url: 'https://i.ibb.co/wJZR9gM/gem-coin.png'}}, start_param: '', ready: () => {}, expand: () => {}}};
    ...
    async function createNewUser() {
        ...
        state = {
            balance: 0, // CHANGED: New user starts with 0 balance
        ...
        if (referrerId && referrerId !== userId && !state.referralBonusClaimed) {
            state.balance = NEW_USER_BONUS; // Grant bonus only if referred
            const referrerRef = database.ref('users/' + referrerId);
            const referrerSnapshot = await referrerRef.get();
            if (referrerSnapshot.exists()) {
                const referrerData = referrerSnapshot.val();
                referrerData.balance = (referrerData.balance || 0) + REFERRER_BONUS;
        ...
        dom.leaderboardContainer.innerHTML = users.map((user, i) => `
            <div class="list-item leaderboard-item ${user.id == userId ? 'is-user' : ''}">
                ...
                <div class="item-info leaderboard-info">
                    <span class="leaderboard-name">${user.fullName || 'A User'}</span>
