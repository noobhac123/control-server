document.addEventListener('DOMContentLoaded', () => {
    ...
    const REFERRER_BONUS = 1000; // CHANGED from 5000 to 1000
    const UPGRADES = {
    ...
    async function createNewUser() {
        ...
        state = {
            ...
            fullName: fullName || user.username || 'A User' // Store fullName
        };
    ...
    async function saveUserData() { 
        const user = tg.WebApp.initDataUnsafe.user;
        const fullName = `${user.first_name || ''} ${user.last_name || ''}`.trim();
        state.fullName = fullName || user.username || 'A User'; // Ensure fullName is always up-to-date
