...
function main(tg) {
    ...
    async function initializeApp() {
        ...
        try {
            ...
            setupUI();
            setupInteractions();
            calculateProfitPerHour(); // Calculate profit first
            
            // NEW LOGIC: If user has profit, set last claim to make vault full
            if (state.profitPerHour > 0) {
                 const fourHoursAgo = Date.now() - (VAULT_CAPACITY_HOURS * 3600 * 1000);
                 if (state.lastClaimTimestamp > fourHoursAgo) {
                    state.lastClaimTimestamp = fourHoursAgo;
                 }
            }
    ...
    async function createNewUser() {
        ...
        if (referrerId && referrerId !== userId && !state.referralBonusClaimed) {
            ...
            state.referralBonusClaimed = true; // Mark bonus as claimed for the new user

            const referrerRef = database.ref('users/' + referrerId);
            try {
                // Use a transaction to safely update the referrer's balance
    ...
    // The rest of the file is unchanged. I am including the full code for completeness.
