document.addEventListener('DOMContentLoaded', () => {
    ...
    const tg = window.Telegram || { WebApp: { initDataUnsafe: { user: { id: '123_LOCAL_TEST', first_name: 'Test', last_name: 'User', username: 'testuser', photo_url: 'https://i.ibb.co/wJZR9gM/gem-coin.png'}}, start_param: 'referrer_12345', ready: () => {}, expand: () => {}}};
    
    const userId = tg.WebApp.initDataUnsafe.user ? tg.WebApp.initDataUnsafe.user.id.toString() : null;
    const referrerId = tg.WebApp.start_param || null;
    ...
    async function initializeApp() {
        if (!userId) { handleError("Could not identify user."); return; }
        try {
            await loadUserData();
            ...
            setupUI(); setupInteractions(); calculateProfitPerHour(); renderAll();
            setInterval(updateVault, 1000);
        } catch (error) {
            console.error("Initialization failed:", error);
            handleError("Failed to load your data.");
        }
    }
    ...
    async function loadUserData() {
        ...
        if (snapshot.exists()) {
            state = snapshot.val();
            // Initialize fields for returning users
    ...
    async function createNewUser() {
        state = {
            balance: NEW_USER_BONUS, // Start with the new user bonus
        ...
        if (referrerId && referrerId !== userId && !state.referralBonusClaimed) {
            ...
            if (referrerSnapshot.exists()) {
                ...
                referrerData.balance += REFERRER_BONUS;
                await database.ref('users/' + referrerId).set(referrerData);
                console.log(`Awarded ${REFERRER_BONUS} to referrer ${referrerId}`);
            }
            state.referralBonusClaimed = true; // Mark bonus as claimed
        }
        await saveUserData();
    }
    ...
    function handleError(message) { dom.splashScreen.innerHTML = `<h1 style="color: red; text-align: center; padding: 20px;">${message}</h1>`; }
    function setupUI() {
    ...
    function setupInteractions() {
        ...
        dom.upgradeList.addEventListener('click', e => { const button = e.target.closest('.upgrade-button'); if(button) handleUpgrade(button.dataset.id); });
        dom.dailyRewardContainer.addEventListener('click', e => { const dayElement = e.target.closest('.claimable'); if(dayElement) handleDailyRewardClaim(dayElement.dataset.day); });
    ...
    function formatNumber(num) {
        ...
        const suffixes = ["", "K", "M", "B", "T"]; const i = Math.floor(Math.log10(num) / 3);
    ...
    async function handleClaim() {
        ...
        if (accumulated >= 1) { state.balance += accumulated; state.lastClaimTimestamp = Date.now(); await saveUserData(); renderAll(); updateVault(); }
    }
    ...
    async function handleUpgrade(id) {
        ...
        if (state.balance >= cost) { state.balance -= cost; state.upgrades[id].level++; calculateProfitPerHour(); await saveUserData(); renderAll(); }
    }
    function renderDailyRewards() {
        dom.dailyRewardContainer.innerHTML = ''; const now = Date.now();
        const hoursSinceLastClaim = (now - state.lastDailyRewardClaim) / (1000 * 3600);
        if (hoursSinceLastClaim > 48) state.dailyRewardStreak = 0;
        for (let i = 0; i < 7; i++) {
            const dayDiv = document.createElement('div'); dayDiv.className = 'daily-reward-day'; dayDiv.dataset.day = i + 1;
            let status = 'locked';
    ...
    async function renderLeaderboard() {
        ...
        let users = [];
        snapshot.forEach(child => { users.push({ id: child.key.toString(), ...child.val() }); });
        users.reverse();
        dom.leaderboardContainer.innerHTML = users.map((user, i) => `
            <div class="list-item leaderboard-item ${user.id == userId ? 'is-user' : ''}">
                <span class="leaderboard-rank">${i + 1}</span><div class="item-info leaderboard-info"><span class="leaderboard-name">${user.id == userId ? 'You' : 'A User'}</span>
                <span class="leaderboard-balance">${formatNumber(user.balance)} 💎</span></div></div>`).join('') || '<p class="text-muted">Be the first on the leaderboard!</p>';
    }
    function updateWithdrawalSummary() {
        const amount = parseFloat(dom.withdrawAmount.value) || 0; const fee = (amount * WITHDRAWAL_FEE_PERCENT) / 100;
        dom.withdrawFee.textContent = formatNumber(fee); dom.withdrawFinal.textContent = formatNumber(amount - fee);
    }
