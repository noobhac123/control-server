document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadUserData() {
        ...
        if (snapshot.exists()) {
            state = snapshot.val();
            // Initialize new fields for returning users
            ...
            state = {
                ...
                dailyRewardStreak: 0, lastDailyRewardClaim: 0,
    ...
    function setupInteractions() {
        ...
        dom.upgradeList.addEventListener('click', e => e.target.closest('.upgrade-button') && handleUpgrade(e.target.dataset.id));
        dom.dailyRewardContainer.addEventListener('click', e => e.target.closest('.claimable') && handleDailyRewardClaim(e.target.closest('.claimable').dataset.day));
    ...
    function renderDailyRewards() {
        ...
        if (hoursSinceLastClaim > 48) state.dailyRewardStreak = 0; // Reset streak
        
        for (let i = 0; i < 7; i++) {
            const dayDiv = document.createElement('div');
            dayDiv.className = 'daily-reward-day';
            ...
            dayDiv.innerHTML = `
                ...
                ${status === 'claimed' ? '<span class="day-claimed-check">✓</span>' : ''}
            `;
            dom.dailyRewardContainer.appendChild(dayDiv);
        }
    }
    
    async function handleDailyRewardClaim(day) {
        day = parseInt(day) - 1;
        state.balance += DAILY_REWARDS[day];
        ...
        dom.leaderboardContainer.innerHTML = users.map((user, i) => `
            <div class="list-item leaderboard-item ${user.id == userId ? 'is-user' : ''}">
                ...
                <div class="item-info leaderboard-info">
                    <span class="leaderboard-name">${tg.WebApp.initDataUnsafe.user.id == user.id ? 'You' : 'A User'}</span>
    ...
    async function handleWithdrawal() {
        ...
        const withdrawalRequest = { amount, address, network: dom.withdrawNetwork.value, status: 'Pending', timestamp: Date.now() };
        await database.ref('withdrawals/' + userId).push(withdrawalRequest);
        await saveUserData();
    ...
    async function renderTransactionHistory() {
        const snapshot = await database.ref('withdrawals/' + userId).limitToLast(10).get();
        if (!snapshot.exists()) { dom.transactionHistoryContainer.innerHTML = '<p class="text-muted">No transactions yet.</p>'; return; }
        let historyHtml = '';
        snapshot.forEach(child => {
            ...
            historyHtml = `
                <div class="list-item">
                    <div class="item-info">
                        <h4>${formatNumber(tx.amount)} 💎 on ${tx.network}</h4>
        ...
        dom.transactionHistoryContainer.innerHTML = historyHtml;
    }
