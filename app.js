document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Prepare mock data for local browser testing
    const mockWebApp = {
        initDataUnsafe: {
            user: {
                id: '123_LOCAL_TEST',
            ...
            start_param: 'referrer_12345'
        },
        ready: () => {},
        expand: () => {}
    };

    // 3. Use real Telegram object or mock object
    const tg = isTelegram ? window.Telegram.WebApp : mockWebApp;
...
function main(tg) {
    ...
    async function initializeApp() {
        ...
        try {
            ...
            handleError("Failed to load your data. Please check your internet and try again.");
        }
    }
        ...
        if (referrerId && referrerId !== userId && !state.referralBonusClaimed) {
            ...
            try {
                const result = await referrerRef.transaction(currentData => {
                    if (currentData === null) {
                        return currentData; // Referrer doesn't exist.
                    }
    ...
    function calculateAccumulatedEarnings() {
        const elapsedTimeInSeconds = (Date.now() - (state.lastClaimTimestamp || Date.now())) / 1000;
    ...
    function renderDailyRewards() {
        ...
        const hoursSinceLastClaim = (now - (state.lastDailyRewardClaim || 0)) / (1000 * 3600);
