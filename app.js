...
function main(tg) {
    ...
    const BOOSTS = {
        ...
        'full-vault': {
            name: 'Full Vault', icon: '💰',
            description: 'Instantly fills your vault to its maximum capacity.',
            durationHours: 0, cost: 2500 // Instant
        }
    };
    ...
    async function handleBuyBoost(id) { const boost = BOOSTS[id]; if (!boost || state.balance < boost.cost) return; const activeBoost = state.activeBoosts?.[id]; if (activeBoost && Date.now() < activeBoost.expiresAt) return; state.balance -= boost.cost; if (id === 'full-vault') { const vaultCapacityMillis = VAULT_CAPACITY_HOURS * 3600 * 1000; state.lastClaimTimestamp = Date.now() - vaultCapacityMillis; updateVault(); } else if (id === 'turbo-profit') { if (!state.activeBoosts) state.activeBoosts = {}; state.activeBoosts[id] = { expiresAt: Date.now() + boost.durationHours * 3600 * 1000 }; calculateProfitPerHour(); } await saveUserData(); renderAll(); renderBoosts(); }
